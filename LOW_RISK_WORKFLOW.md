...
| Generative Ink | **Validated and active; explicitly low risk.** The public home, posts index, and representative long post render as first-party static pages with no frames; the home has a small animated/image surface and the post has two images. | Complete and audit `data/generative-ink-pilot.txt`, then prepare the next low-risk source. |
| alien.v01d.zone | **Validated and N+1-ready; explicitly low risk.** The canonical public material is a single long-form page with approximately 220,166 visible characters, 15 images, no iframe/video/audio, and fragment-only same-origin navigation. | Capture and audit the prepared one-route `data/alien-v01d-pilot.txt` after the active Generative Ink boundary, preserving the static/image and telemetry checks. |
