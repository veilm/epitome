...
| Anthropic careers and jobs | **Complete pilot and thirty-six continuations: 547/547 public route pages; explicitly low risk.** The public `/careers/jobs` index, six pilot details, and thirty-six 15-route Greenhouse continuations completed and are tab-closed after a four-route repair for the original `next-15aj` CDP acquisition failures. The combined result has no unresolved capture-level failures, clean primary/all-rendered-image audits, zero excluded-host results, classified public application/telemetry/PDF/font records, and loopback-only representative replays. The current index snapshot contains 590 versioned job IDs; four Fellows/application-form-heavy listings remain held out, and 40 ordinary public identities remain uncaptured after the repaired batch. | Continue the versioned public-only inventory with the active deduplicated `data/anthropic-careers-next-15ak.txt`; keep application links as references only and never open application/authenticated routes. |
