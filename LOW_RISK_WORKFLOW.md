...
| Greg Brockman blog | **Pilot complete; full capture approved.** The seven-route public pilot completed with `finish.failures=0`, zero response-body errors, clean primary/all-rendered-image audits, zero interactive media, zero asset failures/exclusions, no prohibited-host result URLs, and 144-request local-only representative replays with no live-origin fallback. | Capture the full deduplicated 20-route scope; the X link remains a reference only. |
| Sam Altman blog | **Approved low-risk source; pilot ready.** The rendered 13-page public listing sequence exposes 121 unique canonical posts, with public archive/feed/listing routes retained for structural verification, for a reviewed 136-route scope. | Run the bounded public pilot, then capture the full deduplicated 136-route scope; keep Posthaven administrative/authentication paths out of scope. |
...
| OpenAI careers and jobs | **Pilot plus incremental refresh complete: 78/78 public batches/routes, explicitly low risk.** The original seven-route pilot, the deduplicated 70-route `/index/` refresh, and the one-page reconciliation delta completed with `finish.failures=0`, all manifests tab-closed, clean primary/all-rendered-image audits, zero prohibited-host result URLs, classified optional 401/403/202/pending records, and representative loopback replays with no production-origin requests. The reconciliation page retained 136 completed assets with zero failures/exclusions and six optional response-level body-error records; no YouTube/Twitter/X downloader was used. | Preserve stable job IDs and refresh history; no new OpenAI identity remains in the reconciled source snapshot. |
...
| Claude blog and research | **Incremental refresh complete: 29/29 public posts, explicitly low risk.** The final live-list check found 29 candidates: 28 were duplicates of the prior refresh list and were skipped, while `what-1-000-small-business-owners-taught-us-about-ai` was captured at `/mnt2/capsule/epitome/claude/refresh/1789156195-final-reconcile`. Its manifest is complete/tab-closed with `finish.failures=0`, both image audits are clean, 18 new assets completed with one exact `www.youtube.com` policy exclusion and no failed or prohibited-host result, and a substantive local replay used only loopback resources. The post-capture all-source plan-only gate found no genuinely new identity; all 29 candidates now deduplicate as complete. | Refresh plan 1789091412 is closed; start the next cycle from a new reviewed live-list snapshot and keep historical uncaptured Claude backlog out of ordinary refresh scope. |
...
| SemiAnalysis newsletter | **Incremental refresh complete: 10/10 public posts, explicitly low risk.** The nine-page delta and one-page reconciliation completed with `finish.failures=0`, all manifests tab-closed, zero asset failures, clean primary/all-rendered-image audits, six classified response-level records, zero prohibited-host result URLs, and local-only early/media/structural/late replays preserving the public paid boundary. | Preserve the reviewed delta and paid-preview boundary; the final all-source reconciliation found no new SemiAnalysis identity. |
