...
| Astral Codex Ten | **Seven bounded production batches complete: 420 pages, plus a five-page pilot.** The reviewed first-party archive contains 1,452 canonical posts: 1,170 public and 282 paid-only preview identities. The batches preserve long articles, images, comments, media references, and the paid-preview boundary. The seventh batch has 105/105 complete and tab-closed pages with zero page failures; its 48 dependency-asset failures and 74 response-body errors were classified, both image audits were clean, and representative paid-preview, ordinary, links-heavy, comment-heavy, and media/reference replays made no production-origin requests. Exact YouTube/Twitter/X asset exclusions recorded two skipped decisions and no excluded-host asset result; one incidental YouTube-nocookie page dependency was retained as a network-only source reference, with no downloader invoked. | Start the prepared 120-page list at `data/astral-codex-ten-next-120.txt` with the port-2103-only workflow, the access boundary, bounded asset policy, and exact YouTube/Twitter/X host exclusions; audit before growing again. |
