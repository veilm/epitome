...
| Anthropic careers and jobs | **Complete pilot and twenty-seven continuations: 412/412 public route pages; explicitly low risk.** The public `/careers/jobs` index, six pilot details, and twenty-seven 15-route Greenhouse continuations completed with `finish.failures=0`, no capture-level failures, clean primary/all-rendered-image audits, zero excluded-host results, classified public retirement/application/telemetry/PDF routing records, and loopback-only representative replays. The current index snapshot contains 590 versioned job IDs; four Fellows/application-form-heavy listings remain held out, 175 ordinary public identities were uncaptured before the prepared batch, and 160 remain after it. | Continue the versioned public-only inventory with the active deduplicated `data/anthropic-careers-next-15ab.txt`; keep application links as references only and never open application/authenticated routes. |
