...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, and bounded batches through the next 75 identities are approved. The following 90 are capturing with Gravatar completion excluded. | **Ready.** `data/slate-star-codex-next-105.txt` contains the following 105 identities. Continue using `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
