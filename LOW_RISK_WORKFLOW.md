...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, and bounded batches through the next 180 identities are complete. The following 195 are capturing with Gravatar completion excluded and a true wall-clock asset deadline. | **Ready.** After the active list, the final 193 identities are lines 1,370–1,562 of `sources/slate-star-codex.txt`; the existing-root filter makes that full source file safe as the final input. Continue using `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
