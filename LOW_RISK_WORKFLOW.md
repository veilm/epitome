...
| Anthropic careers and jobs | **Complete pilot and thirty-nine continuations: 592/592 public route pages; explicitly low risk.** The public `/careers/jobs` index, six pilot details, and thirty-nine 15-route Greenhouse continuations completed and are tab-closed after the four-route `next-15aj` repair. The combined result has no unresolved capture-level failures, clean primary/all-rendered-image audits, zero excluded-host results, classified public application/telemetry/PDF/font records, and loopback-only representative replays. The current index snapshot contains 590 versioned job IDs; the ordinary public-only inventory is exhausted, while four Fellows/application-form-heavy listings remain intentionally held out and application links remain references only. | Preserve the completed public-only inventory. No further explicitly prepared low-risk batch is queued; keep application links as references only and never open application/authenticated routes. |
