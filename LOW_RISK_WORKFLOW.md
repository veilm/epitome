...
| Anthropic careers and jobs | **Complete pilot and fifteen continuations: 232/232 public routes; explicitly low risk.** The public `/careers/jobs` index, six pilot details, and fifteen 15-route Greenhouse continuations completed with `finish.failures=0`, no capture-level failures, clean primary/all-rendered-image audits, zero excluded-host results, classified optional application/telemetry dependencies and routing records, and loopback-only representative replays. The current index snapshot contains 590 versioned job IDs; four Fellows/application-form-heavy listings remain held out and 340 public identities remain after the prepared batch. | Continue the versioned public-only inventory with the active deduplicated `data/anthropic-careers-next-15p.txt`; keep application links as references only and never open application/authenticated routes. |
