...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, and bounded batches through the next 90 identities are approved. The following 105 are capturing with Gravatar completion excluded. | **Ready.** `data/slate-star-codex-next-120.txt` contains the following 120 identities. Continue using `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
