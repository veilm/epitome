...
| Anthropic careers and jobs | **Complete pilot and ten continuations: 157/157 public routes; explicitly low risk.** The public `/careers/jobs` index, six pilot details, and ten 15-route Greenhouse continuations completed with `finish.failures=0`, no capture-level failures, clean primary/all-rendered-image audits, zero excluded-host results, classified optional application/telemetry dependencies and routing records, and loopback-only representative replays. The current index snapshot contains 590 versioned job IDs; four Fellows/application-form-heavy listings remain held out and 415 public identities remain queued. | Continue the versioned public-only inventory with the active deduplicated `data/anthropic-careers-next-15k.txt`; keep application links as references only and never open application/authenticated routes. |
