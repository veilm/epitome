...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, and bounded batches through the next 60 identities are approved. The following 75 are capturing with Gravatar completion excluded. | **Ready.** `data/slate-star-codex-next-90.txt` contains the following 90 identities. Continue using `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
