...
| OpenAI careers and jobs | **Pilot plus incremental refresh complete: 77/77 public batches/routes, explicitly low risk.** The original seven-route pilot and the deduplicated 70-route `/index/` refresh completed with `finish.failures=0`, all manifests tab-closed, clean primary/all-rendered-image audits, zero prohibited-host result URLs, classified optional 401/403/202/pending records, and representative loopback replays with no production-origin requests. The refresh recorded 44 hydrated Vimeo references and 639 same-origin optional-static 403 asset failures; no YouTube/Twitter/X downloader was used. | Preserve stable job IDs and refresh history; the reviewed 15-identity Anthropic delta is the active N+1 handoff. |
| Anthropic careers and jobs | **Baseline complete; incremental refresh N+1 ready.** The public `/careers/jobs` index, six pilot details, and thirty-nine 15-route Greenhouse continuations completed at 592/592 public route pages after the four-route `next-15aj` repair. The ordinary baseline inventory contains 590 versioned job IDs, with four Fellows/application-form-heavy listings intentionally held out and application links remaining references only. The reviewed refresh adds 15 newly listed public Greenhouse identities in `/mnt2/capsule/epitome/refresh/1789091412/anthropic.txt`; it is the next sole capture lane after OpenAI. | Capture the reviewed 15-route public-only delta, then continue to the prepared Claude 28-route delta. Keep application links as references only and never open application/authenticated routes. |
