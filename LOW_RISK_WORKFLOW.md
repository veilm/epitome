...
| Gwern.net | **Complete pilot: 7/7; N+1-ready and explicitly low risk.** The bounded public index/about, blog/document indexes, long essay, fiction, and recent blog routes completed with zero capture, response, asset, or exclusion failures. Candidate missing images, public PDF references, zero interactive media, and the direct-video `/embryo-selection` deferral were classified; both image audits and loopback-only replays passed the substantive-content/public-boundary checks. | Capture and audit the prepared twelve-route major-essay family in ignored `data/gwern-major-essays-next-12.txt` before expanding the blog/document families. |
...
| OpenAI careers and jobs | **Validated and N+1-ready; explicitly low risk.** The public careers index exposes 768 jobs and the varied detail pilot is public, substantive, and free of login/paywall/media boundaries; six stable Ashby IDs are recorded for versioned tracking. | Capture and audit the prepared seven-route public-only `data/openai-careers-pilot.txt` before expanding the job inventory. |
