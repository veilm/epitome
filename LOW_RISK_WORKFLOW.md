...
| Import AI by Jack Clark | **Validated and N+1-ready; explicitly low risk.** The public Substack home and a representative article expose substantive available text within the preview/paywall boundary; platform frames and optional broken preview images are recorded as bounded risks. | Capture and audit the prepared five-route public-only `data/import-ai-pilot.txt`, preserving the access boundary and classifying frames, previews, images, and attachments. |
...
| near.blog and its link graph | **Validated and active; explicitly low risk.** The public home and six representative WordPress routes are substantive and server-rendered; the bounded pilot includes a 69-image list and a known missing-image post, while the direct-video `/this-anime-does-not-exist/` route is deferred to a separate media scope. | Complete and audit `data/near-blog-pilot.txt`, then inventory outbound provenance from the links page and bound recursive discovery. |
