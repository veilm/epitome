...
| --- | --- | --- |
| LessWrong | **Partial.** Taxonomy and the 1,384-post core/curated inventory are complete. The seven-page pilot, all 51 rendered Highlights identities, all 19 Janus identities, and the nine-route priority pilot pass capture/image checks; the priority pilot's sequence-index placeholders are classified and its first 15 Fun Theory member posts are now active as N. A further 15-member batch is prepared as N+1. | Complete and audit `data/lesswrong-priority-members-next-15.txt`; then start `data/lesswrong-priority-members-next-15b.txt` and continue the ordered sequence expansion by stable ID. |
