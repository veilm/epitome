...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, and bounded batches through the next 165 identities are complete. The following 180 are capturing with Gravatar completion excluded and a true wall-clock asset deadline. | **Ready.** `data/slate-star-codex-next-195.txt` contains the following 195 identities. Continue using `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
