...
| --- | --- | --- |
| LessWrong | **Partial.** Taxonomy and the 1,384-post core/curated inventory are complete. The seven-page pilot, all 51 rendered Highlights identities, and all 19 Janus identities pass offline replay; Janus has zero capture-level failures and clean primary/all-image audits. The reviewed priority source now has a bounded nine-route pilot prepared. | Start the bounded priority pilot from `data/lesswrong-priority-pilot.txt` with the same public-only, port-2103, 15-second-settle policy; after it passes, expand the ordered Fun Theory, 6BF, and Highly Advanced Epistemology members by stable ID. |
