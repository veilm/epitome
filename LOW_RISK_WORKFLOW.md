...
| Cyborgism Wiki | **Validated and N+1-ready; explicitly low risk.** The public home and representative `/hypha/cyborgism` page render as stable first-party wiki pages with no frames or account requirement. | Start the seven-route `data/cyborgism-wiki-pilot.txt` after the Citrini boundary, with wiki, list, category, profile, bibliography, and image checks. |
...
| Citrini Research article | **Validated and active; explicitly low risk.** Canonical page `https://www.citriniresearch.com/p/2028gic` rendered publicly with a substantive article, 26 images, and no access-control bypass. | Complete and audit `data/citrini-research-pilot.txt`, then advance to the prepared Cyborgism Wiki pilot. |
