...
| Slate Star Codex | **Active.** The 1,562-identity canonical scope, five representative variants, first 15, and next 30 identities are approved. The next 45 are capturing with Gravatar completion excluded. | **Ready.** `data/slate-star-codex-next-60.txt` contains the following 60 identities. Use `--exclude-asset-host secure.gravatar.com`; require primary images and comment text, not complete avatar presentation. |
