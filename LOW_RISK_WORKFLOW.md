...
| Astral Codex Ten | **Three bounded batches complete: 90 pages.** The reviewed first-party archive contains 1,452 canonical posts: 1,170 public and 282 paid-only preview identities. The batches preserve long articles, images, comments, media-bearing content, and the paid-preview boundary. The third batch has 45/45 complete and tab-closed pages, four classified dependency-asset failures, and six response-body errors that did not affect page capture; primary/all-image audits and loopback replay are clean. Two small YouTube embed documents and one Twitter-hosted image were incidental generic-asset records, so the next run excludes their exact hosts without invoking media downloads. | Start the prepared 60-page list at `data/astral-codex-ten-next-60.txt` with the port-2103-only workflow, the access boundary, bounded asset policy, and exact YouTube/Twitter host exclusions; audit before growing again. |
