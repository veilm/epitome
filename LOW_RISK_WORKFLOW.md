...
| Cyborgism Wiki | **Validated and active; explicitly low risk.** The public home and representative `/hypha/cyborgism` page render as stable first-party wiki pages with no frames or account requirement. | Complete and audit `data/cyborgism-wiki-pilot.txt`, then advance to the prepared Generative Ink pilot. |
| Generative Ink | **Validated and N+1-ready; explicitly low risk.** The public home, posts index, and representative long post render as first-party static pages with no frames; the home has a small animated/image surface and the post has two images. | Start the seven-route `data/generative-ink-pilot.txt` after the Cyborgism boundary, with static/animated, index, long-post, image, and category checks. |
