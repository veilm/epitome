...
| Astral Codex Ten | **First bounded batch complete: 15 pages.** The reviewed first-party archive contains 1,452 canonical posts: 1,170 public and 282 paid-only preview identities. The pilot and first batch preserve long articles, images, comments, and the paid-preview boundary; representative replay is loopback-only. Two external cited PDFs returned 403, but primary and all-image audits are clean. | Start the prepared 30-page list at `data/astral-codex-ten-next-30.txt` on port 2103 with the same bounded asset policy, then audit before growing again. |
