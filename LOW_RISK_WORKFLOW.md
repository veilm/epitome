...
| Gwern.net | **Not started; next preparation target.** Preliminary research identifies a large first-party site, so it must be divided into bounded families. | Inventory major essays, blog entries, and first-party documents; prepare a varied pilot rather than crawling the whole link graph at once. |
