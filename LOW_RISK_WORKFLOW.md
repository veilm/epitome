...
| Anthropic careers and jobs | **Complete pilot: 7/7; explicitly low risk.** The public `/careers/jobs` index and six varied public Greenhouse detail pages completed with `finish.failures=0`, zero response or asset failures, clean primary/all-rendered-image audits, zero excluded-host results, and representative loopback replays with no production-origin requests. The current index snapshot contains 590 unique job IDs. | Capture the prepared deduplicated 15-route `data/anthropic-careers-next-15.txt`; keep the four held-out Fellows/application-form pages outside the ordinary public page lane. |
