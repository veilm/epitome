...
| --- | --- | --- |
| LessWrong | **Partial.** Taxonomy and the 1,384-post core/curated inventory are complete. The seven-page pilot, all 51 rendered Highlights identities, and all 19 Janus identities pass offline replay; Janus has zero capture-level failures and clean primary/all-image audits. The reviewed nine-route priority pilot is now active, with its first 15 stable Fun Theory member posts prepared as N+1. | Complete and audit `data/lesswrong-priority-pilot.txt`; then start the reserved `data/lesswrong-priority-members-next-15.txt` batch and continue the ordered sequence expansion by stable ID. |
