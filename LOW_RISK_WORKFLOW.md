...
| Anthropic careers and jobs | **Validated and N+1-ready; explicitly low risk.** The public `/careers/jobs` index rendered 590 unique Greenhouse job IDs, and six varied public detail pages rendered substantive text with intact images; application references were not followed. The ignored seven-route public-only pilot is prepared. | Capture and offline-audit `data/anthropic-careers-pilot.txt`, then expand only through versioned job-identity snapshots. |
