...
| --- | --- | --- |
| LessWrong | **Partial.** Taxonomy and the 1,384-post core/curated inventory are complete. The seven-page pilot and all 51 rendered Highlights identities pass offline replay. Janus's rendered public author profile exposes 19 stable post identities; all 19 are new after deduplication, and the varied seven-page pilot is complete with zero capture-level failures, clean image audits, and loopback-only representative replays. | Start the bounded N+1 capture from `data/lesswrong-janus-next-12.txt`, using the same public-only, port-2103, 15-second-settle policy; then classify it before defining a broader core/curated tier. |
