...
| Paul Graham essays | Page crawl active | 215/232 approved identities are complete across the structural index, varied pilots, and clean bounded batches. All sampled replays preserve the narrow early-web layout and images without production requests. Exact Wayback copies restore the retired Hacker News comment badge and a linked historical traffic chart. Catalog dates retain the source's month precision instead of inventing a day. | Capture the prepared final 17 essays in `data/paul-graham-final-17.txt`. Recover the unavailable outbound *Roots of Lisp* PDF separately, and keep books/RSS/site utilities outside the essay scope. |
