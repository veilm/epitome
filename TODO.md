...
- [ ] **Company careers and job listings**
  ...
  - [x] Capture and offline-audit the `next-15b` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, clean
        image audits, classified optional application/telemetry dependencies,
        no excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15c.txt`.
  - [x] Capture and offline-audit the `next-15c` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, clean primary/all-rendered-image audits,
        classified optional application/telemetry dependencies, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15d.txt`; keep application links
        as references only and preserve the four held-out Fellows listings.
  - [x] Capture and offline-audit the `next-15d` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, clean primary/all-rendered-image audits,
        classified optional application/telemetry dependencies, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15e.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15e` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, clean primary/all-rendered-image audits,
        classified optional application/telemetry dependencies, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15f.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15f` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, clean primary/all-rendered-image audits,
        classified optional application/telemetry dependencies, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15g.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15g` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        ...
        representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15h.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15h` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, clean primary/all-rendered-image audits,
        classified the single redirect boundary record, no excluded-host
        results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15i.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15i` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the single public Greenhouse
        redirect/status boundary, clean primary/all-rendered-image audits,
        no excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15j.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15j` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-
        image audits, no excluded-host results, and loopback-only
        representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15k.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15k` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-
        image audits, no excluded-host results, and loopback-only
        representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15l.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15l` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-
        image audits, no excluded-host results, and loopback-only
        representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15m.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15m` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the three public Greenhouse
        redirect/status boundaries, clean primary/all-rendered-image audits,
        no excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15n.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15n` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the two public Greenhouse
        redirect/status boundaries, clean primary/all-rendered-image audits,
        no excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15o.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15o` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the two public Greenhouse
        redirect/status boundaries, clean primary/all-rendered-image audits,
        no excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15p.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15p` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the public Greenhouse retirement
        redirect and optional application-surface statuses, clean
        primary/all-rendered-image audits, no excluded-host results, and
  ...
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ...
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15r` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the retired public Greenhouse
        redirect/status boundary, clean primary/all-rendered-image audits, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15s.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15s` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level failures, classified the retired public Greenhouse
        redirect/status boundary, clean primary/all-rendered-image audits, no
        excluded-host results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15t.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15t` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, classified the public PDF
        dependency, clean primary/all-rendered-image audits, no excluded-host
        results, and loopback-only representative replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15u.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15u` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-image
        audits, no excluded-host results, and loopback-only representative
        replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15v.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15v` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-image
        audits, no excluded-host results, and loopback-only representative
        replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15w.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15w` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-image
        audits, no excluded-host results, and loopback-only representative
        replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15x.txt`; continue the archived
        index without opening application or authenticated routes.
  - [x] Capture and offline-audit the `next-15x` versioned Anthropic
        job-identity batch: 15/15 public routes, `finish.failures=0`, no
        capture-level or response-body failures, clean primary/all-rendered-image
        audits, no excluded-host results, and loopback-only representative
        replays.
  - [x] Prepare and start the next deduplicated public-only Anthropic batch in
        ignored `data/anthropic-careers-next-15y.txt`; continue the archived
        index without opening application or authenticated routes.
